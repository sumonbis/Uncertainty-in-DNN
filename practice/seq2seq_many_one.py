...
@tf.function
def train_step(target_seq_in, target_seq_out, en_initial_states):
    with tf.GradientTape() as tape:

        de_outputs = decoder(target_seq_in, en_initial_states)
        logits = de_outputs[0]
...
def predict():
    ...
    en_initial_states = decoder.init_states(1)
    en_outputs = decoder(tf.constant([[fr_tokenizer.word_index['<start>']]]), en_initial_states)

    # de_input = tf.constant([[fr_tokenizer.word_index['<start>']]])
    # de_input=tf.constant(test_source_seq)
    de_input = tf.constant([test_source_seq[0][0:1]])
    # de_state_h, de_state_c = en_initial_states
    ...
    while True:
        ...
        de_input = tf.constant([test_source_seq[0][i-1:i]])
        i+=1
...
for e in range(NUM_EPOCHS):
    en_initial_states = decoder.init_states(BATCH_SIZE)

    for batch, (target_seq_in, target_seq_out) in enumerate(dataset.take(-1)):
        loss = train_step(target_seq_in,
